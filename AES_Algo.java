...
//				}
				...
				byte[] stg = new byte[16];
				SecureRandom number = new SecureRandom();
				number.nextBytes(stg);
				IvParameterSpec iv = new IvParameterSpec(stg);

				Cipher c = Cipher.getInstance("AES/CBC/PKCS5Padding");
				c.init(Cipher.ENCRYPT_MODE, key_s, iv);
				byte[] opencrypt = Base64.getEncoder().encode(c.doFinal(m));

				String output = new String(opencrypt, "UTF-8");
				System.out.println("Enrypted message is below.");
				System.out.println(output);

				c.init(Cipher.DECRYPT_MODE, key_s, iv);
				byte[] opdecrypt = c.doFinal(Base64.getDecoder().decode((output)));
				System.out.println("\nDecrypted message is below.");
				System.out.print(new String(opdecrypt, "UTF-8"));
				...
				Cipher c2 = Cipher.getInstance("AES/OFB/PKCS5Padding");
				c2.init(Cipher.ENCRYPT_MODE, key_s, iv);
				byte[] ofb = Base64.getEncoder().encode(c2.doFinal(m));

				String ofben = new String(ofb, "UTF-8");
				System.out.println("Enrypted message is below.");
				System.out.println(ofben);

				// Decrypt

				c2.init(Cipher.DECRYPT_MODE, key_s, iv);
				byte[] ofbde = c2.doFinal(Base64.getDecoder().decode((ofben)));
				System.out.println("\nDecrypted message is below.");
				System.out.print(new String(ofbde, "UTF-8"));
